...
class Program
{
    ...
    static void Main()
    {
        ...
        // var byEmail = debtors.FindAll(e => e.Email.EndsWith("rhyta.com") || e.Email.EndsWith("dayrep.com"));
        // byEmail.ForEach(e => Console.WriteLine(e));

        //3) Yashi 26 - dan 36 - ya qeder olan borclulari cixartmag

        // var byAge = debtors.FindAll(a => a.BirthDay.Year >= 1986 && a.BirthDay.Year <= 1996);
        //byAge.ForEach(a => Console.WriteLine(a));

        //4) Borcu 5000 - den cox olmayan borclularic cixartmag

        // var byMoney = debtors.FindAll(m => m.Debt <= 5000);
        // byMoney.ForEach(m => Console.WriteLine(m));

        //5) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2 - den cox 7 reqemi olan borclulari cixartmaq

        // var bySimvol = debtors.FindAll(s => s.FullName.Length > 18 && s.Phone.Split('7').Length>=2);
        //bySimvol.ForEach(s => Console.WriteLine(s));

        //7) Qishda anadan olan borclulari cixardmaq

        // var byMonth = debtors.FindAll(m => m.BirthDay.Month == 12 && m.BirthDay.Day >= 22 || m.BirthDay.Month == 1 || m.BirthDay.Month <= 2 || m.BirthDay.Month ==3 && m.BirthDay.Day <=22);
        // byMonth.ForEach(m => Console.WriteLine(m));
        ...
        //var byMoney = debtors.FindAll(d => d.Debt > debtors.Average<Debtor>(d => d.Debt)).OrderBy(d => d.FullName).ToList();
        //byMoney.ForEach(d => Console.WriteLine(d));
        //Console.WriteLine("---------------------------------------------------------------------------------------------------");
        //byMoney = byMoney.OrderByDescending(d => d.Debt).ToList();
        //byMoney.ForEach(d => Console.WriteLine(d));

        //9) Telefon nomresinde 8 olmayan borclularin yalniz familyasin, yashin ve umumi borcun meblegin cixarmaq

        //var byPhone = debtors.FindAll(p => p.Phone.Split('8').Length == 1);
        //byPhone.ForEach(s => Console.WriteLine($"{s.FullName} {s.BirthDay} {s.Debt}"));
        ...
        //var byName = debtors.FindAll(n => n.FullName.Any(m =>n.FullName.Split(m).Length >= 3)).OrderBy(m => m.FullName).ToList();
        //byName.ForEach(s => Console.WriteLine(s));

        //3)borclulardan en coxu hansi ilde dogulubsa hemin ili cixartmaq

        var byYear = debtors.GroupBy(g => g.BirthDay.Year).OrderByDescending(gr => gr.Count()).Select(s=>s.Key).FirstOrDefault();
        Console.WriteLine(byYear);
    
        //14)Borcu en boyuk olan 5 borclunun siyahisini cixartmaq

        //var byDebt = debtors.OrderByDescending(d => d.Debt).Take(5).ToList();
        //byDebt.ForEach(d => Console.WriteLine(d));

        //15)Butun borcu olanlarin borcunu cemleyib umumi borcu cixartmaq

        //var sum = debtors.Sum(s => s.Debt);
        //Console.WriteLine(sum);

        //16)2ci dunya muharibesin gormush borclularin siyahisi cixartmaq

        //var byYear = debtors.FindAll(y => y.BirthDay.Year < 1945);
        //byYear.ForEach(d => Console.WriteLine(d));

        //18)Nomresinde tekrar reqemler olmayan borclularin ve onlarin borcunun meblegin cixartmaq

        //var byPhone = debtors.FindAll(p => p.Phone.Distinct().ToList().Count == 11);
        //byPhone.ForEach(d => Console.WriteLine(d));
        ...
        //var byDebt = debtors.FindAll(d => d.Debt - ((DateTime.Now.DayOfYear >= d.BirthDay.DayOfYear) ? 365 - DateTime.Now.DayOfYear + d.BirthDay.DayOfYear : d.BirthDay.DayOfYear - DateTime.Now.DayOfYear) / 30 * 500 <= 0);
        //byDebt.ForEach(d => Console.WriteLine(d));

        //20)Adindaki ve familyasindaki herflerden "smile" sozunu yaza bileceyimiz borclularin siyahisini cixartmaq

        //var byName = debtors.FindAll(n =>
        //{
        //    var fn = n.FullName.ToLower();
        //    return fn.Contains('s') && fn.Contains('m') && fn.Contains('i') && fn.Contains('l') && fn.Contains('e');
        //}).ToList();
        //byName.ForEach(d => Console.WriteLine(d));
    }
}